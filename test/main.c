...
static void test_klciph()
{
	...
	LOG("1. ");
	assert(klciph_enc(NULL, 1, cipher, &clen) == FAIL);
	assert(klciph_enc(plain, 0, cipher, &clen) == FAIL);
	assert(klciph_enc(plain, 1, cipher, &clen) == SUCC);
	assert(klciph_enc(plain, 120, cipher, &clen) == SUCC);
	assert(klciph_enc(plain, 121, cipher, &clen) == FAIL);
	assert(klciph_enc(plain, strlen(plain), NULL, &clen) == FAIL);
	assert(klciph_enc(plain, strlen(plain), cipher, NULL) == FAIL);
	assert(klciph_enc(plain, strlen(plain), cipher, &clen) == SUCC);
	uint8_t plain2[256];
	int plen2;
	assert(klciph_dec(NULL, clen, plain2, &plen2, 256) == FAIL);
	assert(klciph_dec(cipher, 0, plain2, &plen2, 256) == FAIL);
	assert(klciph_dec(cipher, 1, plain2, &plen2, 256) == FAIL);
	assert(klciph_dec(cipher, 128, plain2, &plen2, 256) == FAIL);
	assert(klciph_dec(cipher, 255, plain2, &plen2, 256) == FAIL);
	assert(klciph_dec(cipher, 256, plain2, &plen2, 256) == SUCC);
	assert(klciph_dec(cipher, clen, NULL, &plen2, 256) == FAIL);
	assert(klciph_dec(cipher, clen, plain2, NULL, 256) == FAIL);
	assert(klciph_dec(cipher, clen, plain2, &plen2, 256) == SUCC);
	plain2[plen2] = 0;
